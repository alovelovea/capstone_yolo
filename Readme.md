...
| 구분         | 내용                                                                       |
| ---------- | ------------------------------------------------------------------------ |
| 객체 인식 모델   | YOLOv11 기반 쓰레기 객체 탐지 모델 학습 및 실시간 추론 기능 구현                                |
| 데이터셋 구축    | AIHub, Roboflow 등 공개 데이터와 팀 자체 촬영 데이터를 결합하여 학습 데이터셋 구성                   |
| 데이터 전처리    | 이미지 라벨링, 클래스 정리, Train/Validation 분할, Detection 형식 변환 및 데이터 검증 수행        |
| 탐지 클래스     | 캔, 페트병, 플라스틱, 유리, 종이, 비닐 등 주요 분리배출 대상 객체 탐지                              |
| 실시간 인식     | 노트북에 연결된 웹캠을 통해 컨베이어 벨트 위의 쓰레기 객체를 실시간으로 탐지                              |
| 하드웨어 제어    | YOLO 예측 결과를 MQTT로 NodeMCU에 전송하고, NodeMCU가 서보모터 기반 분류 플랩을 제어              |
| 자동 분류 장치   | 컨베이어 벨트, 웹캠, NodeMCU, 서보모터, 플랩 구조를 활용한 쓰레기 자동 분류 시스템 제작                  |
| GitHub 문서화 | `model/` 및 `hardware/` 폴더를 분리하여 모델 학습 과정, 전처리 과정, 하드웨어 연동 과정을 README로 정리 |
| 최종 제출물     | 모델 코드, 전처리 코드, 실시간 탐지 코드, 하드웨어 제어 코드, README, Release, 발표/시연 자료 정리       |
...
| 이름 | 역할 | 주요 수행 내용 |
|---|---|---|
| 홍태성 | 프로젝트 총괄<br>하드웨어 제작 | 프로젝트 기획 및 보고서 정리<br>캔 데이터 수집·라벨링<br>컨베이어 벨트 제작<br>웹캠 기반 하드웨어 테스트 |
| 박소영 | 데이터 수집<br>문서화 | 플라스틱·종이·비닐 데이터 수집·라벨링<br>데이터셋 보완<br>프로젝트 소개 및 데이터 수집 과정 문서화 |
| 황현석 | 데이터 정리<br>결과 분석 | 페트병 데이터 수집·라벨링<br>AIHub 데이터 정리<br>실시간 탐지 테스트<br>모델 결과 정리 |
| 박민재 | 모델 개발<br>GitHub 정리 | 유리 데이터 수집·라벨링<br>YOLO 모델 학습 및 성능 테스트<br>통합 데이터셋 구성<br>전처리 코드 정리<br>model 폴더 및 README 구성 |
...
```text
https://github.com/alovelovea/capstone_yolo/releases/tag/v1.0.0
```
