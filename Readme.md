...
### 1. 프로젝트 개요
지속 가능한 환경을 조성하기 위해 올바른 분리배출은 필수적이지만, 복잡한 분류 기준으로 인해 실생활에서는 혼동이 자주 발생합니다. 본 프로젝트는 분리배출 과정의 혼동을 줄이고 효율을 높이기 위해, **딥러닝 객체 탐지(Object Detection) 모델을 활용하여 쓰레기 종류를 실시간으로 식별하고 관리하는 시스템 구축**을 목표로 합니다. 이를 통해 사용자의 판단 부담을 줄이고, 보다 정확하고 일관된 분리배출 환경을 실현하고자 합니다.
